...
Try {
	...
	[string]$appName = 'SPSS'
	[string]$appVersion = '28.0.1 Patch'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-MSI -Action "Install" -Path "IBM SPSS Statistics.msi" -Transform "1033.MST" -Parameters "REBOOT=ReallySupress /QN INSTALLPYTHON=`"1`" COMPANYNAME=`"Metropolitan State University of Denver`" $installParameters" -SecureParameters -PassThru

		If (Test-Path -Path "$envProgramFiles\IBM\SPSS Statistics\as-3.3.0.0\lib")
		{
			Remove-File -Path "$envProgramFiles\IBM\SPSS Statistics\log4j-core-2.13.3.jar"
			Remove-File -Path "$envProgramFiles\IBM\SPSS Statistics\log4j-api-2.13.3.jar"
			Remove-File -Path "$envProgramFiles\IBM\SPSS Statistics\log4j-1.2-api-2.13.3.jar"

			Remove-File -Path "$envProgramFiles\IBM\SPSS Statistics\as-3.3.0.0\lib\log4j-core-2.13.3.jar"
			Remove-File -Path "$envProgramFiles\IBM\SPSS Statistics\as-3.3.0.0\lib\log4j-api-2.13.3.jar"
			Remove-File -Path "$envProgramFiles\IBM\SPSS Statistics\as-3.3.0.0\lib\log4j-1.2-api-2.13.3.jar"

			Copy-File -Path "$dirFiles\Patch\log4j-1.2-api-2.15.0.jar" -Destination "$envProgramFiles\IBM\SPSS Statistics\"
			Copy-File -Path "$dirFiles\Patch\log4j-api-2.15.0.jar" -Destination "$envProgramFiles\IBM\SPSS Statistics\"
			Copy-File -Path "$dirFiles\Patch\log4j-core-2.15.0.jar" -Destination "$envProgramFiles\IBM\SPSS Statistics\"

			Copy-File -Path "$dirFiles\Patch\log4j-1.2-api-2.15.0.jar" -Destination "$envProgramFiles\IBM\SPSS Statistics\as-3.3.0.0\lib"
			Copy-File -Path "$dirFiles\Patch\log4j-api-2.15.0.jar" -Destination "$envProgramFiles\IBM\SPSS Statistics\as-3.3.0.0\lib"
			Copy-File -Path "$dirFiles\Patch\log4j-core-2.15.0.jar" -Destination "$envProgramFiles\IBM\SPSS Statistics\as-3.3.0.0\lib"
		}
